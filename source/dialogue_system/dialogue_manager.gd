...
@export var dialogue_box_screen_position: Vector2
...
func _ready() -> void:
	...
	if not dialogue_box_scene:
		push_warning("Dialogue manager: no valid dialogue box.")
		return

	if trigger:
		trigger.player_trigged_dialogue.connect(_on_dialogue_trigged)
		trigger.player_left_dialogue_area.connect(end_dialogue)
	
	dialogue_box = dialogue_box_scene.instantiate() as DialogueTextBox
